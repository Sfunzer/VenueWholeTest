...
public class ReservationStore {          //Declaration of the fields which will contain all reservation objects.
    ...
    public Boolean addReservation(String reservationName, String reservationLocation, String reservationInfo, String madeByUser, String reservationStartTime, String reservationStopTime) {
        boolean dateAlreadyBooked = false;
        LocalDateTime startParse = LocalDateTime.parse(reservationStartTime);
        LocalDateTime stopParse = LocalDateTime.parse(reservationStopTime);

        for (Reservation show : reservationList) {
            if (startParse.toLocalDate().equals(show.reservationStart.toLocalDate())) {
